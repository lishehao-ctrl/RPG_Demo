...
def test_replay_timeline_uses_persisted_state_baseline_not_hardcoded_default(tmp_path: Path, monkeypatch) -> None:
    _prepare_db(tmp_path)
    client = TestClient(app)
    ...
    with db_session.SessionLocal() as db:
        ...
        assert state is not None
        state.score_visible = 73
        state.relation_vector = {'trust': 0.73, 'attraction': 0.22, 'fear': 0.04, 'respect': 0.61}
        db.commit()
        char_id = str(state.character_id)

    from app.modules.session import service as session_service

    def _fixed_affection(*, current_score_visible, relation_vector, drift, behavior_tags):
        new_vec = dict(relation_vector or {})
        return {
            'new_score_visible': current_score_visible + 7,
            'new_relation_vector': new_vec,
            'new_drift': drift or {},
            'rule_hits': [],
            'vector_delta': {'trust': 0.0, 'attraction': 0.0, 'fear': 0.0, 'respect': 0.0},
            'score_delta': 7,
        }

    monkeypatch.setattr(session_service, 'apply_affection', _fixed_affection)

    step = client.post(f'/sessions/{sid}/step', json={'input_text': 'fixed-delta'})
    assert step.status_code == 200
    ...
    first = replay.json()['affection_timeline'][char_id][0]
    assert first['step_index'] == 0
    assert first['score_visible'] == 73
    assert first['relation_vector']['trust'] == 0.73
