...
DEFAULT_CASE_MAX_WORKERS = 12
DEFAULT_TOTAL_RPM_LIMIT = 300
DEFAULT_STANDARD_TIMEOUT_SECONDS = 1800.0
DEFAULT_HEAVY_TIMEOUT_SECONDS = 2400.0
DEFAULT_CASE_AGGREGATE_TIMEOUT_SECONDS = 5400.0
...
def run_gold_eval_full(
    ...
    *,
    ...
    normalized_profile = "heavy" if str(profile).strip().lower() == "heavy" else "standard"
    if normalized_profile == "heavy":
        case_catalog = burst_pressure_realistic_20()
        timeout_seconds = (
            float(case_timeout_seconds)
            if case_timeout_seconds is not None
            else DEFAULT_HEAVY_TIMEOUT_SECONDS
        )
    else:
        case_catalog = v1_topic_gold_realistic_10()
        timeout_seconds = (
            float(case_timeout_seconds)
            if case_timeout_seconds is not None
            else DEFAULT_STANDARD_TIMEOUT_SECONDS
        )
    return run_gold_eval_suite(
        ...
        case_set_filename="full_case_set.json",
        blockers_filename="blockers.md",
        blockers_title=f"Gold Eval Full Blockers ({normalized_profile})",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run full gold eval with forced persona self-play + play_eval + llm_text_audit."
    )
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--profile", choices=("standard", "heavy"), default="standard")
