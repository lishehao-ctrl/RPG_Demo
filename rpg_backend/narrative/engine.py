...
_LANGUAGE_DIRECTIVE_EN = (
    "ALL narration, NPC dialogue, option labels, option hints, "
    "npc_pulse.state, npc_pulse.reason, ending passages, highlights, "
    "branches, advisor replies, oracle hints, and any other free-form "
    "prose MUST be written in fluent, natural English. Do NOT include "
    "any Chinese characters in any free-form text field. Field schema "
    "(key names like 'passage', 'options', 'npc_pulse', 'character_id', "
    "'role_id') stays as defined — only the VALUES change language. "
    "Cast character_id and player_role role_id stay as "
    "lowercase_underscore identifiers (English-friendly slugs). "
    "display_name should be a plausible English given/full name; role "
    "and relation_to_protagonist describe the character in English. "
    "The intent-tag prefix on options must be in English: use "
    "[Provoke] (was [挑拨]) / [Counter] ([反将]) / [Yield] ([妥协]) / "
    "[Watch] ([观望]) / [Probe] ([试探]) / [Ally] ([合作]) / "
    "[Confront] ([硬刚]) / [Submit] ([示弱]). The ending_label is the "
    "ONE field that stays as a canonical Chinese token from the closed "
    "pool (复仇 / 和解 / 自由 / 救赎 / 回归 / 夺回 / 孤狼 / 共谋 / "
    "牺牲 / 同谋 / 决裂 / 沉沦 / 失控 / 反噬 / 破碎) — the UI maps it "
    "to a localized display string. Do NOT translate or invent a new "
    "label. ending_subtitle (the first-person line) IS in English."
)
