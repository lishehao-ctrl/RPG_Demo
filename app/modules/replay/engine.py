...
class ReplayEngine:
    ...
    def build_report(self, session_id: uuid.UUID, db: Session) -> dict:
        ...
        for state in final_states:
            ...
            baseline_score = int(state.score_visible) - score_delta_totals.get(char_id, 0)
            baseline_score = max(0, min(100, baseline_score))
            baseline_vector = {}
            for dim in self.REQUIRED_DIMS:
                baseline_vector[dim] = max(-1.0, min(1.0, float(relation.get(dim, 0.0)) - vector_delta_totals[char_id][dim]))

            accumulator[char_id] = {
                'score_visible': baseline_score,
                'relation_vector': baseline_vector,
            }
            affection_timeline[char_id].append(
                {
                    ...
                    'score_visible': baseline_score,
                    'relation_vector': dict(baseline_vector),
                }
            )
