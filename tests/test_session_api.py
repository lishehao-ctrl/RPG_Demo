...
from app.db.models import ActionLog, DialogueNode, SessionSnapshot
...
def test_rollback_trim_uses_snapshot_membership_not_timestamp_only(tmp_path: Path) -> None:
    ...
    sid = client.post("/sessions").json()["id"]
    first = client.post(f"/sessions/{sid}/step", json={"input_text": "first"})
    assert first.status_code == 200

    snap_id = client.post(f"/sessions/{sid}/snapshot").json()["snapshot_id"]

    second = client.post(f"/sessions/{sid}/step", json={"input_text": "second"})
    assert second.status_code == 200
    second_node_id = second.json()["node_id"]

    with db_session.SessionLocal() as db:
        snap = db.get(SessionSnapshot, uuid.UUID(snap_id))
        cutoff = datetime.fromisoformat(snap.state_blob["cutoff_ts"])
        db.execute(text("UPDATE dialogue_nodes SET created_at=:cutoff WHERE id=:id"), {"cutoff": cutoff, "id": second_node_id})
        db.execute(text("UPDATE action_logs SET created_at=:cutoff WHERE node_id=:id"), {"cutoff": cutoff, "id": second_node_id})
        db.commit()
