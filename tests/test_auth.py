...
def test_dev_invalid_x_user_id_returns_structured_400(tmp_path: Path) -> None:
    _prepare_db(tmp_path)
    old_env = settings.env
    settings.env = "dev"
    try:
        client = TestClient(app)
        resp = client.post("/sessions", headers={"X-User-Id": "not-a-uuid"})
        assert resp.status_code == 400
        detail = resp.json()["detail"]
        assert detail["code"] == "INVALID_X_USER_ID"
    finally:
        settings.env = old_env
